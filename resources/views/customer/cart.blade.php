        ...
        <div class="container flex">
            <div class="flex items-center justify-between flex-grow py-2">
                <div class="flex items-center space-x-6 capitalize">
                    <a href="/" class="text-gray-200 hover:text-red font-[Lora] hover:no-underline font-bold text-[20px] {{ request()->is('/') ? 'text-red-500' : '' }}">Home</a>
                    <a href="/shop" class="text-gray-200 hover:text-red transition font-[Lora] hover:no-underline font-bold text-[20px] {{ request()->is('shop') ? 'text-red-500' : '' }}">Bali Tour Package</a>
                    <a href="/" class="text-gray-200 hover:text-red transition font-[Lora] hover:no-underline font-bold text-[20px] {{ request()->is('car') ? 'text-red-500' : '' }}">Car</a>
                    <!-- <a href="/riwayat-order" class="text-gray-200 hover:text-red transition font-[Lora]">Riwayat Order</a> -->
                    <a href="/" class="text-gray-200 hover:text-red transition font-[Lora] hover:no-underline font-bold text-[20px] {{ request()->is('contact') ? 'text-red-500' : '' }}">Contact Us</a>
                    <a href="/" class="text-gray-200 hover:text-red transition font-[Lora] hover:no-underline font-bold text-[20px] {{ request()->is('about') ? 'text-red-500' : '' }}">About Us</a>
                </div>
                ...
                <div class="col-span-12 space-y-4">
                    <div class="flex items-center justify-between border gap-6 p-4 border-gray-200 rounded">
                        ...
                        <div class="flex items-center">
                            <!-- Tombol Minus -->
                            @if ($c['qty'] > 1)
                                <button onclick="updateQuantity({{ $c['id'] }}, {{ $c['qty'] - 1 }})" class="px-2 py-1 bg-gray-200">-</button>
                            @else
                            ...
                            <span class="px-4">{{ $c['qty'] }}</span>

                            <!-- Tombol Plus -->
                            <button onclick="updateQuantity({{ $c['id'] }}, {{ $c['qty'] + 1 }})" class="px-2 py-1 bg-gray-200">+</button>
                            
                            <div class="container">  
                                <input class="date form-control" type="text" name="date" value="{{ $c['date'] ?? '' }}" onchange="updateDate({{ $c['id'] }}, this.value)"> 
                            </div>
                        </div>
